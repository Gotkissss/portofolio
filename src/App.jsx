...
function Nav() {
  ...
  return (
    <nav style={{
      ...
      <span style={{ fontFamily: 'var(--mono)', fontSize: '0.8rem', color: 'var(--muted)', letterSpacing: '0.1em' }}>DAQ</span>
        ...
        <>
          ...
          {menuOpen && (
            <div style={{
              ...
              display: 'flex', flexDirection: 'column', padding: '1.5rem 1.25rem', gap: '1.25rem',
            }}>
              ...
              <a href="https://github.com/Gotkissss" target="_blank" rel="noreferrer" style={{
                fontFamily: 'var(--mono)', fontSize: '0.85rem', color: 'var(--accent)',
        ...
        <div style={{ display: 'flex', gap: '2rem', alignItems: 'center' }}>
          {[['trabajo', 'work'], ['habilidades', 'skills'], ['sobre mí', 'about']].map(([label, id]) => (
            <a key={id} href={`#${id}`} style={{
              ...
              textDecoration: 'none', letterSpacing: '0.1em', textTransform: 'uppercase', transition: 'color 0.2s',
            }}
  ...
  return (
    <section style={{
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: '900px' }}>
        ...
        <h1 style={{
          fontFamily: 'var(--mono)', fontSize: 'clamp(2.4rem, 7vw, 6rem)',
          fontWeight: 700, lineHeight: 1.0, letterSpacing: '-0.02em',
          color: 'var(--fg)', marginBottom: '0.25rem',
        }}>Diego</h1>
        <h1 style={{
          fontFamily: 'var(--mono)', fontSize: 'clamp(2.4rem, 7vw, 6rem)',
        ...
        <p style={{
          fontFamily: 'var(--sans)', fontSize: isMobile ? '1rem' : '1.1rem',
          fontWeight: 300, color: 'var(--muted)', maxWidth: '520px',
        ...
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
          <a href="#work" style={{
            fontFamily: 'var(--mono)', fontSize: '0.8rem', color: 'var(--bg)',
            background: 'var(--fg)', textDecoration: 'none',
            padding: '0.7rem 1.6rem', letterSpacing: '0.08em', transition: 'background 0.2s',
          }}
          onMouseEnter={e => e.target.style.background = 'var(--accent)'}
          onMouseLeave={e => e.target.style.background = 'var(--fg)'}
          >VER TRABAJO</a>
          <a href="https://github.com/Gotkissss" target="_blank" rel="noreferrer" style={{
            fontFamily: 'var(--mono)', fontSize: '0.8rem', color: 'var(--fg)',
            background: 'transparent', textDecoration: 'none',
            padding: '0.7rem 1.6rem', letterSpacing: '0.08em',
            border: '1px solid var(--line)', transition: 'border-color 0.2s, color 0.2s',
          }}
      ...
      {!isMobile && (
        <div style={{
          ...
          letterSpacing: '0.1em', writingMode: 'vertical-rl', opacity: 0.5,
        }}>DESLIZAR PARA EXPLORAR</div>
      )}
    </section>
  )
}
...
function ProjectCard({ project, index }) {
  ...
  return (
    ...
    >
      <div style={{
        fontFamily: 'var(--mono)', fontSize: '0.7rem', color: 'var(--muted)',
        letterSpacing: '0.1em', paddingTop: isMobile ? 0 : '0.25rem',
      }}>{project.slug}</div>

      <div>
        <h3 style={{
          fontFamily: 'var(--mono)', fontSize: '1.2rem', fontWeight: 500,
          color: hovered ? 'var(--accent)' : 'var(--fg)',
          marginBottom: '1rem', transition: 'color 0.2s',
        }}>{project.title}</h3>
        <p style={{
          fontFamily: 'var(--sans)', fontSize: '0.9rem', color: 'var(--muted)',
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '2rem' }}>
          {project.stack.map(t => (
            <span key={t} style={{
              ...
              border: '1px solid var(--line)', padding: '0.2rem 0.6rem', letterSpacing: '0.06em',
            }}>{t}</span>
          ))}
        </div>
        <div style={{ display: 'flex', gap: '1.5rem' }}>
          ...
          <a href={project.repo} target="_blank" rel="noreferrer" style={{
            fontFamily: 'var(--mono)', fontSize: '0.75rem', color: 'var(--muted)',
            textDecoration: 'none', letterSpacing: '0.1em',
            borderBottom: '1px solid var(--line)', paddingBottom: '1px',
          }}>REPO ↗</a>
        </div>
      </div>
      ...
      {!isMobile && (
        <div>
          <p style={{
            fontFamily: 'var(--mono)', fontSize: '0.68rem', color: 'var(--muted)',
            letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '1rem', opacity: 0.6,
          }}>// destacados</p>
          <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
            {project.highlights.map((h, i) => (
              <li key={i} style={{
                fontFamily: 'var(--mono)', fontSize: '0.78rem', color: 'var(--muted)',
                lineHeight: 1.6, paddingLeft: '1.2rem', position: 'relative', marginBottom: '0.5rem',
              }}>
      ...
      {isMobile && (
        <div>
          <p style={{
            fontFamily: 'var(--mono)', fontSize: '0.68rem', color: 'var(--muted)',
            letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '0.75rem', opacity: 0.6,
          }}>// destacados</p>
          <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
            {project.highlights.map((h, i) => (
              <li key={i} style={{
                fontFamily: 'var(--mono)', fontSize: '0.75rem', color: 'var(--muted)',
                lineHeight: 1.6, paddingLeft: '1.2rem', position: 'relative', marginBottom: '0.4rem',
              }}>
                <span style={{ position: 'absolute', left: 0, color: 'var(--accent)' }}>→</span>
                {h}
              </li>
...
function Work() {
  ...
  const isMobile = useIsMobile()
  return (
    <section id="work" style={{ padding: isMobile ? '5rem 1.25rem' : '8rem 2.5rem', maxWidth: '1100px', margin: '0 auto' }}>
      <div ref={ref} style={{
        display: 'flex', alignItems: 'baseline', gap: '1.5rem', marginBottom: '4rem',
        opacity: inView ? 1 : 0, transform: inView ? 'none' : 'translateY(20px)', transition: 'all 0.5s ease',
        flexWrap: 'wrap',
      }}>
        <h2 style={{
          fontFamily: 'var(--mono)', fontSize: 'clamp(1.6rem, 3vw, 2.5rem)',
          fontWeight: 700, color: 'var(--fg)', letterSpacing: '-0.02em',
        }}>Proyectos Seleccionados</h2>
        <span style={{
          fontFamily: 'var(--mono)', fontSize: '0.75rem', color: 'var(--muted)', letterSpacing: '0.1em',
  ...
  return (
    <section id="skills" style={{
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        <h2 ref={ref} style={{
          fontFamily: 'var(--mono)', fontSize: 'clamp(1.6rem, 3vw, 2.5rem)',
          fontWeight: 700, color: 'var(--fg)', letterSpacing: '-0.02em', marginBottom: '4rem',
          opacity: inView ? 1 : 0, transform: inView ? 'none' : 'translateY(20px)', transition: 'all 0.5s ease',
        }}>Tecnologías</h2>
        <div style={{
          display: 'grid',
            ...
            <div key={category}>
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                {items.map(item => (
                  <li key={item} style={{
                    fontFamily: 'var(--mono)', fontSize: '0.85rem', color: 'var(--muted)',
                    lineHeight: 1.9, borderBottom: '1px solid var(--line)', paddingBottom: '0.1rem',
                  }}>{item}</li>
                ))}
              </ul>
            </div>
...
function About() {
  ...
  const isMobile = useIsMobile()
  return (
    <section id="about" style={{ padding: isMobile ? '5rem 1.25rem' : '8rem 2.5rem' }}>
      <div style={{
        ...
        <div>
          <h2 style={{
            fontFamily: 'var(--mono)', fontSize: 'clamp(1.6rem, 3vw, 2.5rem)',
            fontWeight: 700, color: 'var(--fg)', letterSpacing: '-0.02em', marginBottom: '2rem',
          }}>Sobre mí</h2>
          <p style={{
            fontFamily: 'var(--sans)', fontSize: '1rem', color: 'var(--muted)',
            lineHeight: 1.8, fontWeight: 300, marginBottom: '1.25rem',
          }}>
